...
document.addEventListener('DOMContentLoaded', () => {
  fetch('events.json')
    ...
    .then(data => {
      applyAdminOverrides(data); // overlay any links saved by admin.html
      buildPage(data);
    })
    .catch(err => {
      console.error(err);
...
/**
 * Reads link overrides saved by admin.html from localStorage
 * and applies them on top of events.json before the page builds.
 * Key format: "categoryId__eventIndex"  e.g. "technical__0"
 */
function applyAdminOverrides(data) {
  try {
    const saved = JSON.parse(localStorage.getItem('eminence_links') || '{}');
    if (!Object.keys(saved).length) return;
    data.categories.forEach(cat => {
      cat.events.forEach((ev, i) => {
        const key = `${cat.id}__${i}`;
        if (saved[key]) ev.link = saved[key];
      });
    });
  } catch (e) {
    console.warn('Admin overrides could not be applied:', e);
  }
}
